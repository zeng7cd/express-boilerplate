...
- [7. 缓存系统](#7-缓存系统)
- [8. 事件系统](#8-事件系统)
- [9. Service 层架构](#9-service-层架构)
- [10. Repository 模式](#10-repository-模式)
- [11. 错误处理](#11-错误处理)
- [12. 最佳实践](#12-最佳实践)
- [13. 注意事项](#13-注意事项)
...
// 日期
z.date().min(new Date('2020-01-01'), 'Date too early').max(new Date(), 'Date cannot be in future');

// 枚举
z.enum(['admin', 'user', 'guest']);

// 数组
z.array(z.string()).min(1, 'At least one item required').max(10, 'Maximum 10 items').nonempty('Array cannot be empty');
...
export class CacheWarmupService {
  async warmupAll(): Promise<void> {
    await Promise.all([this.warmupRoles(), this.warmupPermissions(), this.warmupSettings()]);
  }
...
Service 层是业务逻辑的核心，位于 Controller 和 Repository 之间，负责：

- **业务逻辑编排**: 协调多个 Repository 完成复杂业务
- **事务管理**: 处理跨多个表的事务操作
- **业务规则验证**: 实现业务规则和约束
- **事件发布**: 发布业务事件供其他模块订阅
- **缓存管理**: 管理业务数据的缓存策略
...
Controller (HTTP 层)
    ↓
Service (业务逻辑层) ← 使用类 + 依赖注入
    ↓
Repository (数据访问层) ← 使用类 + 继承
    ↓
Database (数据库)
```

### 9.2 为什么 Service 层使用类

**使用类的优势**:

1. **依赖注入**: 通过构造函数注入依赖，便于测试和维护
2. **状态封装**: 可以管理服务级别的状态（如 logger、配置）
3. **代码组织**: 相关方法组织在一起，职责清晰
4. **易于测试**: 可以轻松 mock 依赖进行单元测试
5. **类型安全**: TypeScript 提供完整的类型推断和检查

**对比函数式**:

```typescript
// ❌ 不推荐 - 函数式 Service（难以管理依赖）
...
export class UserService {
  ...
  async getUserById(id: string) {
    ...
    if (cached) {
      this.logger.debug({ userId: id }, 'User found in cache');
      return cached;
    }
   ...
   * 私有方法 - 验证业务规则
   */
  private async validateUpdateData(id: string, data: UpdateUserDto) {
    // 业务规则验证逻辑
    if (data.email) {
...
export const getUserService = (): UserService => {
  if (!userServiceInstance) {
    ...
    userServiceInstance = new UserService(userRepository, cacheService);
  }
...
export const getMyService = (): MyService => {
  if (!serviceInstance) {
    ...
    serviceInstance = new MyService(dependency1, dependency2);
  }
  return serviceInstance;
};

// 向后兼容的导出
export const myService = new Proxy({} as MyService, {
  get(_target, prop) {
    return getMyService()[prop as keyof MyService];
  },
});
```
...
describe('UserService', () => {
  ...
  it('should get user from cache', async () => {
    ...
    vi.mocked(mockCacheService.get).mockResolvedValue(cachedUser);
  ...
  it('should get user from database when cache miss', async () => {
    const dbUser = { id: '1', name: 'John' };
    vi.mocked(mockCacheService.get).mockResolvedValue(null);
    vi.mocked(mockUserRepository.findById).mockResolvedValue(dbUser);

    const result = await userService.getUserById('1');

    expect(result).toEqual(dbUser);
    expect(mockUserRepository.findById).toHaveBeenCalledWith('1');
    expect(mockCacheService.set).toHaveBeenCalledWith('user:1', dbUser, 600);
  });
});
```

### 9.6 Service 层 vs Repository 层

**职责划分**:

| 层级       | 职责                     | 示例                           |
| ---------- | ------------------------ | ------------------------------ |
| Controller | HTTP 请求处理、响应格式化 | 解析请求、调用 Service、返回响应 |
| Service    | 业务逻辑、事务、缓存     | 验证规则、编排操作、发布事件   |
| Repository | 数据访问、查询构建       | CRUD 操作、复杂查询            |
...
export class CustomException extends AppException {
  constructor(message: string, details?: any) {
    super(message, 'CUSTOM_ERROR_CODE', StatusCodes.BAD_REQUEST, details);
  }
}
...
6. **缓存系统** - Redis 缓存和装饰器
7. **事件系统** - 类型安全的事件总线
8. **Service 层架构** - 业务逻辑层设计和依赖注入
9. **Repository 模式** - 数据访问层抽象
10. **错误处理** - 统一的错误处理机制
11. **最佳实践** - 代码组织和性能优化
12. **注意事项** - 常见问题和解决方案
...
**更新日志**:
- v1.1 (2025-12-17): 添加 Service 层架构章节，详细说明依赖注入和最佳实践
