...
import { UserEvents } from '../event-types';
import type { UserRegisteredEvent, UserLoginEvent, UserLogoutEvent } from '../event-types';
...
export function initUserEventHandlers(): void {
  ...
  // 处理用户登录事件
  eventBus.subscribe<UserLoginEvent>(UserEvents.LOGIN, async (data) => {
  ...
  // 处理用户登出事件
  eventBus.subscribe<UserLogoutEvent>(UserEvents.LOGOUT, async (data) => {
